...
if __name__ == "__main__":
    # Load the dataset
    file_path = 'annotatedGemma7B_2.csv'
    file_path_1 = 'variation_annotated_queries.csv'

    data = pd.read_csv(file_path)

    # Use existing 'difficulty' column to separate queries
    data['hardness_level'] = data['difficulty']
    ...
    for category in categories:
        ...
        print(f'Results for {category.capitalize()} saved to {output_path}')

        # Print the results
        print(f'{category.capitalize()} Exact Match Percentage: {percentage_exact_match:.2f}%')
        print(f'{category.capitalize()} Partial Match Percentage: {average_partial_match:.2f}%')
    ...
    rects2 = ax.bar([p + bar_width for p in index], partial_matches, bar_width, color='green', label='Partial Matches')
